...
class BootstrapHtmlHelper extends HtmlHelper {
    ...
    protected $_defaultConfig = [
        'templates' => [
            ...
            'link' => '<a class="{{btnClass}}" href="{{url}}"{{attrs}}>{{content}}</a>',
	...
	if(isset($options['btn'])){
		$options['type'] = 'button';
		$btnClass = 'btn';
		if(isset($options['btn-size']) && $options['btn-size']) $btnClass .= '-' . $options['btn-size'];
		if($options['btn'] !== true && $options['btn']) $btnClass .= ' btn-' . $options['btn'];
		unset($options['btn']);
	}
    ...
    public function btnLink($title, $url = null, array $options = [])
    {
        $escapeTitle = true;
        if ($url !== null) {
            $url = $this->Url->build($url);
	...
	if(isset($options['size'])){
		$btnClass .= $options['size'];
	}

	if(isset($options['style'])){
		$btnClass .= ' btn-' . $options['style'];
		unset($options['style']);
	}else{
    ...
    public function cimage($model, $field, array $options = []){
       $path = $model->{$field . '_dir'} . DS . $model->{$field};
       //$path = str_replace('img' . DS . 'webroot' . DS, '', $path);
       $path = parent::image($path, $options);
       $path = str_replace(DS . 'img' . DS . 'webroot', '', $path);
       return $path;
    }
}
